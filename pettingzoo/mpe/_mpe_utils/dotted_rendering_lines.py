...
def _add_attrs(geom, attrs):
    ...
    if "linedotted" in attrs:
        geom.set_linedotted(attrs["linedotted"])
...
def make_circle(radius=10, res=30, filled=True, dotted=False):
    points = []
    for i in range(res):
        ang = 2 * math.pi * i / res
    ...
    if filled:
        ...
        return PolyLine(points, True, dotted)
...
class PolyLine(Geom):
    def __init__(self, v, close, dotted):
        Geom.__init__(self)
        self.v = v
        self.close = close
        ...
        if(dotted):
            self.linedotted = LineStyle(0x08)
            self.add_attr(self.linedotted)
